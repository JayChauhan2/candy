...
export const RivalBattleMap = ({ rival, onBack, startIntro = true }: { rival: Rival; onBack: () => void; startIntro?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const addOutpost=(x:number,z:number)=>{const g=new THREE.Group();g.position.set(x,0,z);world.add(g);const base=new THREE.Mesh(new THREE.CylinderGeometry(.9,1.1,.32,8),mat(0x777a74));base.position.y=.16;g.add(base);[[-.58,-.42],[.58,-.42],[-.58,.42],[.58,.42]].forEach(([px,pz])=>{const leg=new THREE.Mesh(new THREE.CylinderGeometry(.07,.1,2.1,6),mat(0x765139));leg.position.set(px,1.2,pz);g.add(leg)});const deck=new THREE.Mesh(new THREE.BoxGeometry(1.55,.18,1.3),mat(0x8c6745));deck.position.y=2.28;g.add(deck);const roof=new THREE.Mesh(new THREE.ConeGeometry(1.16,.82,4),mat(0x477cb2));roof.position.y=3.15;roof.rotation.y=Math.PI/4;g.add(roof)}
