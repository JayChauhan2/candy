...
export const RivalBattleMap = ({ rival, onBack, startIntro = true }: { rival: Rival; onBack: () => void; startIntro?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const addCastle = (x: number, z: number, colors: { stone: number; roof: number; flag: number }) => {
      ...
      const pole = new THREE.Mesh(new THREE.CylinderGeometry(.05, .05, 3, 6), mat(0x744b32)); pole.position.set(0, 6.1, 0); g.add(pole); const flag = new THREE.Mesh(new THREE.PlaneGeometry(1.55, .7), new THREE.MeshBasicMaterial({ color: colors.flag, side: THREE.DoubleSide })); flag.position.set(.78, 7.05, 0); g.add(flag); return g
    }
    ...
    const playerX = -28, rivalX = rival === 'Queen Marigold' ? 43 : 30, rivalOffset = rival === 'Queen Marigold' ? 26 : 0, rotateQueen = rival === 'Queen Marigold'
    const queenPoint=(x:number,z:number):[number,number]=>rotateQueen?[rivalX-z,x-rivalX]:[x,z]
    addHomeCastle(world,mat,playerX,0); const rivalCastle=addCastle(rivalX, 0, { stone: config.stone, roof: config.roof, flag: config.accent });if(rotateQueen)rivalCastle.rotation.y=Math.PI/2
    addRoad(playerX + 4.1, 0, rivalX - 4.1, 0, 2.15); addRoad(playerX, -3.8, playerX - 5, -10, .9);{const [rx1,rz1]=queenPoint(rivalX,3.8),[rx2,rz2]=queenPoint(rivalX+5,10);addRoad(rx1,rz1,rx2,rz2,.9)}
    ...
    ;[[10, -9], [23, 8], [27, -7], [14, 10]].forEach(([x, z], i => { const [px,pz]=queenPoint(x + rivalOffset,z); const house=new THREE.Group();house.position.set(px,.05,pz);house.rotation.y=rotateQueen?Math.PI/2:0;world.add(house);const body=new THREE.Mesh(new THREE.CylinderGeometry(.9,1.05,1.5,8),mat(0xdabf87));body.position.y=.78;house.add(body);const roofMesh=new THREE.Mesh(new THREE.ConeGeometry(1.34,1.45,6),mat(i%2?config.roof:config.leaf));roofMesh.position.y=2.2;roofMesh.castShadow=true;house.add(roofMesh) }))
    ...
    const move = (event: PointerEvent) => { if (!dragging) return; const dx = event.clientX - lastX, dy = event.clientY - lastY, speed = .05 / zoom; targetX = Math.max(-46, Math.min(46, targetX - dx * Math.cos(yaw) * speed - dy * Math.sin(yaw) * speed)); lastX = event.clientX; lastY = event.clientY }
