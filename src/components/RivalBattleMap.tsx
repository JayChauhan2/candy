...
export const RivalBattleMap = ({ rival, onBack, startIntro = true }: { rival: Rival; onBack: () => void; startIntro?: boolean }) => {
  ...
  useEffect(() => {
    ...
    addCastle(playerX, 0, { stone: 0xb7b5ad, roof: 0x597d9f, flag: 0xf0c14d }); addCastle(rivalX, 0, { stone: config.stone, roof: config.roof, flag: config.accent })
    addRoad(playerX + 4.1, 0, rivalX - 4.1, 0, 2.15); addRoad(playerX, -3.8, playerX - 5, -10, .9); addRoad(rivalX, 3.8, rivalX + 5, 10, .9)
    const addOutpost=(x:number,z:number)=>{const g=new THREE.Group();g.position.set(x,0,z);world.add(g);const base=new THREE.Mesh(new THREE.CylinderGeometry(1.36,1.66,.52,8),mat(0x777a74));base.position.y=.34;g.add(base);[[-.9,-.62],[.9,-.62],[-.9,.62],[.9,.62]].forEach(([px,pz])=>{const leg=new THREE.Mesh(new THREE.CylinderGeometry(.1,.13,3.12,6),mat(0x8c6745));leg.position.set(px,1.9,pz);g.add(leg)});const deck=new THREE.Mesh(new THREE.BoxGeometry(2.25,.22,1.78),mat(0x8c6745));deck.position.y=3.8;g.add(deck);const room=new THREE.Mesh(new THREE.BoxGeometry(2.04,1.32,1.4),mat(0x916f50));room.position.y=4.54;g.add(room);const roof=new THREE.Mesh(new THREE.ConeGeometry(1.61,.94,4),mat(0x477cb2));roof.position.y=5.66;roof.rotation.y=Math.PI/4;g.add(roof);const pole=new THREE.Mesh(new THREE.CylinderGeometry(.05,.06,2.12,6),mat(0x704929));pole.position.set(.74,6.74,0);g.add(pole);const flag=new THREE.Mesh(new THREE.PlaneGeometry(1.1,.58),new THREE.MeshBasicMaterial({color:0x477cb2,side:THREE.DoubleSide}));flag.position.set(1.29,7.28,0);g.add(flag)}
    ...
    savedMap.forEach((item,index)=>{if(typeof item.x!=='number'||typeof item.z!=='number')return;const x=playerX+item.x*.42,z=item.z*.42;if(item.type==='house')addHouse(x,z,index%2?0xc85b43:0x627492);if(item.type==='station1')addOutpost(x,z);if(item.type==='house'||item.type==='station1')addRoad(playerX,0,x,z,.7)})
