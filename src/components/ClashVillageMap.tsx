...
export const mountClashVillageScene = (host: HTMLDivElement) => {
    ...
    // A live troop showcase sits beside the capital so character ideas can be
    // judged in the actual game world rather than as disconnected icons.
    const troopShowcase:{group:THREE.Group;legs:THREE.Mesh[];phase:number;cx:number;cz:number;speed:number;facing:number}[]=[]
    const addTroop=(kind:'shield'|'spear'|'mage'|'archer',x:number,z:number,phase:number)=>{
      ...
      if(kind==='shield'){const shield=new THREE.Mesh(new THREE.CylinderGeometry(.38,.38,.1,8),makeMaterial(0xe4c45f));shield.rotation.x=Math.PI/2;shield.position.set(.34,.75,.08);group.add(shield);const helm=new THREE.Mesh(new THREE.ConeGeometry(.38,.35,7),makeMaterial(0x6c8aa4));helm.position.y=1.54;group.add(helm)}
      ...
      troopShowcase.push({group,legs,phase,cx:x,cz:z,speed:.8+(phase%3)*.06,facing:0})
    }
