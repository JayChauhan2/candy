...
export const mountClashVillageScene = (host: HTMLDivElement) => {
    ...
    const placeBuilderMonument=(type:BuilderType,x:number,z:number,isPreview=false)=>{
      ...
      let buttonHeight=3
      if(type==='house'){add(new THREE.CylinderGeometry(1.02,1.16,1.42,8),0xe0bd78,.8);add(new THREE.ConeGeometry(1.42,1.28,7),0xc85b43,2.12);buttonHeight=3.2}
      else if(type.startsWith('station')){const variant=Number(type.slice(7)),roofColors=[0x477cb2,0x386ba3,0x5d6fa8,0xd65443,0x527c9c,0x426a91,0x6f8fa0,0x486e9a,0x5b6f9e,0x315f99],woodColors=[0x8c6745,0x765139,0x9b704b,0x704929,0x916f50,0x79543c,0x9b7b58,0x6f5848,0x755038,0x826048],towerHeight=2.7+(variant%4)*.42,deck=2.25+(variant%3)*.24;add(new THREE.CylinderGeometry(1.36,1.66,.52,8),0x777a74,.34);[[-.9,-.62],[.9,-.62],[-.9,.62],[.9,.62]].forEach(([px,pz])=>{const leg=add(new THREE.CylinderGeometry(.1,.13,towerHeight,6),woodColors[variant-1],.34+towerHeight/2);leg.position.set(px,.34+towerHeight/2,pz)});add(new THREE.BoxGeometry(deck,.22,1.78),woodColors[variant-1],towerHeight+.68);if(variant===4||variant===8){add(new THREE.CylinderGeometry(1.04,1.2,1.36,8),0xa39273,towerHeight+1.42);add(new THREE.ConeGeometry(1.52,.94,6),roofColors[variant-1],towerHeight+2.5)}else{add(new THREE.BoxGeometry(1.74+(variant%2)*.3,1.2+(variant%3)*.12,1.4),0x916f50,towerHeight+1.42);const roof=add(new THREE.ConeGeometry(1.45+(variant%2)*.16,.94,4),roofColors[variant-1],towerHeight+2.54);roof.rotation.y=Math.PI/4}const door=add(new THREE.BoxGeometry(.46,.74,.07),0x71483a,towerHeight+1.18);door.position.z=.73;const rearWindow=add(new THREE.BoxGeometry(.3,.34,.06),0x86b6c5,towerHeight+1.54);rearWindow.position.z=-.73;[-1,1].forEach(side=>{const sideWindow=add(new THREE.BoxGeometry(.06,.34,.3),0x86b6c5,towerHeight+1.54);sideWindow.position.x=side*.94});if(variant===2||variant===5||variant===7){const deckWing=add(new THREE.BoxGeometry(.96,.13,1.08),woodColors[variant-1],towerHeight+.88);deckWing.position.x=-1.38}if(variant===6||variant===9){add(new THREE.ConeGeometry(.3,.74,6),0xe85c2e,towerHeight+1.15).position.z=.88}const pole=add(new THREE.CylinderGeometry(.05,.06,2.12,6),0x704929,towerHeight+3.62);pole.position.x=.74;const flag=new THREE.Mesh(new THREE.PlaneGeometry(1.1,.58),new THREE.MeshBasicMaterial({color:roofColors[variant-1],side:THREE.DoubleSide}));flag.position.set(1.29,towerHeight+4.16,0);group.add(flag);buttonHeight=towerHeight+4.85}
    ...
    const move = (event:PointerEvent) => {
      ...
      rotationButtons.forEach(button=>{const {x,z,deleteButton}=button.userData as {x:number;z:number;deleteButton:THREE.Sprite};button.getWorldPosition(controlPosition).project(camera);const nearRotate=button.visible&&Math.hypot((controlPosition.x-pointerNdc.x)*rect.width*.5,(controlPosition.y-pointerNdc.y)*rect.height*.5)<180;deleteButton.getWorldPosition(controlPosition).project(camera);const nearDelete=button.visible&&Math.hypot((controlPosition.x-pointerNdc.x)*rect.width*.5,(controlPosition.y-pointerNdc.y)*rect.height*.5)<180;const visible=Math.hypot(point.x-x,point.z-z)<4.3||nearRotate||nearDelete;button.visible=visible;deleteButton.visible=visible})
    }
    ...
    const upgradeClick=(event:PointerEvent)=>{const rect=host.getBoundingClientRect();raycaster.setFromCamera(new THREE.Vector2((event.clientX-rect.left)/rect.width*2-1,-((event.clientY-rect.top)/rect.height*2-1)),camera);const control=closestVisibleControl([...deleteButtons,...rotationButtons],event,rect);if(control&&deleteButtons.includes(control)){const remove=control;if(!remove.userData.confirming){remove.userData.confirming=true;(remove.material as THREE.SpriteMaterial).map=confirmTexture;(remove.material as THREE.SpriteMaterial).needsUpdate=true;return}const monument=remove.userData.monument as THREE.Group;island.remove(monument);rotationButtons.filter(button=>button.userData.monument===monument).forEach(button=>scene.remove(button,button.userData.deleteButton as THREE.Sprite));for(let i=rotationButtons.length-1;i>=0;i--)if(rotationButtons[i].userData.monument===monument)rotationButtons.splice(i,1);for(let i=deleteButtons.length-1;i>=0;i--)if(deleteButtons[i].userData.monument===monument)deleteButtons.splice(i,1);return}if(control&&rotationButtons.includes(control)){const monument=control.userData.monument as THREE.Group;monument.rotation.y+=Math.PI/2;return}const hit=raycaster.intersectObjects(upgradeButtons,false)[0];if(hit)upgradeStructure(hit.object as THREE.Sprite)}
